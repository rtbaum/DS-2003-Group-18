...
## Grouped Bar Chart for BodyPart by Days_Missed (HOMEWORK CHART #1)

```{r}
plotlya1 <- question1data %>%
...
```{r}
ui <-shinyUI(fluidPage(selectInput("selectPlot", "Choose desired plot", choices=paste0("plotlya", 1:4)), plotlyOutput("plot")))
...
## Grouped Bar Chart for BodyPart by Days_Missed (HOMEWORK CHART #2)

```{r}
b1plotly <- question2data %>%
  mutate(fType = as.factor(BodyPart)) %>%
  count(fType, Days_Missed) %>%
  plot_ly(x = ~fType, y = ~n, color = ~Days_Missed, colors = pal,
